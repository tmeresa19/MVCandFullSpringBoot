package com.tigist.loginregistration.controllers;
...
import com.tigist.loginregistration.services.UserService;
...
public class UserController {
	...
	public String processLogin(@Valid @ModelAttribute("newLogin")LoginUser newLogin, BindingResult result, Model model, HttpSession session){
		...
		if(result.hasErrors()) {
			...
			session.setAttribute("userId", loginUser.getId());//unlike for register, for login, we don't save anything except saving the using info in session and redirect to pages with resources. However, before displaying the resource to the logged in user, first need to check the user's id in session in the @GetMapping("/welcome") route for route protection)
			return "redirect:/welcome";
		}
